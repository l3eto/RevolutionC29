var data = [
    {
      'artist':'Kike Pavón',
      'song':'La Experiencia de Adorar',
      'order':[
        'intro',
        'verse-1',
        'verse-2',
        'chorus-1',
        'intro',
        'verse-1',
        'verse-2',
        'chorus-1',
        'chorus-2',
        'bridge',
        'bridge',
        'bridge',
        'chorus-1',
        'chorus-2',
        'intro'
      ],
      'lyrics':{
        'verse-1':[
          {'p':'Es mejor un día en tu presencia,'},
          {'p':'que mil años fuera de ella,'},
          {'p':'pues nada se compara a ella'}
        ],
        'verse-2':[
          {'p':'Es mejor un rincón en tu casa,'},
          {'p':'que tener el mejor palacio,'},
          {'p':'mejor estar en tu regazo'}
        ],
        'chorus-1':[
          {'p':'Y adorar, desde hoy hasta la eternidad'},
          {'p':'y en tu presencia descansar,'},
          {'p':'pues no hay mejor lugar que estar donde tú estás'}
        ],
        'chorus-2':[
          {'p':'Y habitar, bajo tus alas contemplar,'},
          {'p':'tú gran poder y tu bondad'},
          {'p':'y disfrutar de la experiencia de adorarte Jesús'}
        ],
        'bridge':[
          {'p':'Quiero vivir en tu presencia,'},
          {'p':'quiero adorar hasta que no tenga fuerzas'}
        ]
      },
      'chords':{
        'intro':[
          {'4':'C'},
          {'4':'G'},
          {'4':'Am'},
          {'4':'F'}
        ],
        'verse-1':[
          {'4':'C'},
          {'4':'Em'},
          {'4':'Am'},
          {'4':'F'}
        ],
        'verse-2':[
          {'4':'C'},
          {'4':'Em'},
          {'4':'Am'},
          {'4':'F'}
        ],
        'chorus-1':[
          {'4':'C'},
          {'4':'Am'},
          {'4':'G'},
          {'4':'F'}
        ],
        'chorus-2':[
          {'4':'C'},
          {'4':'Am'},
          {'4':'G'},
          {'4':'F'}
        ],
        'bridge':[
          {'4':'Dm'},
          {'4':'C'},
          {'4':'G'},
          {'4':'F'}
        ]
      }
    }
    ,
    {
      'artist':'Evan Craft',
      'song':'Gracia Sublime Es',
      'order':[
        'intro',
        'verse-1',
        'verse-2',
        'chorus-1',
        'chorus-2',
        'intro',
        'verse-3',
        'verse-4',
        'chorus-1',
        'chorus-2',
        'bridge-1',
        'bridge-1',
        'bridge-1',
        'bridge-2',
        'chorus-1',
        'chorus-2'
      ],
      'lyrics':{
        'verse-1':[
          {'p':'Quién rompe el poder del pecado,'},
          {'p':'su amor es fuerte y poderoso,'},
          {'p':'el Rey de gloria, el Rey de majestad'}
        ],
        'verse-2':[
          {'p':'Conmueve el mundo con su estruendo'},
          {'p':'y nos asombra con maravillas,'},
          {'p':'el Rey de gloria, el Rey de majestad'}
        ],
        'verse-3':[
          {'p':'Pusiste en orden todo el caos,'},
          {'p':'nos adoptaste como tus hijos,'},
          {'p':'el Rey de gloria, el Rey de majestad'}
        ],
        'verse-4':[
          {'p':'El que gobierna con su justicia'},
          {'p':'y resplandece con su belleza,'},
          {'p':'el Rey de gloria, el Rey de majestad'}
        ],
        'chorus-1':[
          {'p':'Gracia sublime es,'},
          {'p':'perfecto es tu amor,'},
          {'p':'tomaste mi lugar,'},
          {'p':'cargaste tu mi cruz'}
        ],
        'chorus-2':[
          {'p':'Tu vida diste ahí'},
          {'p':'y ahora libre soy,'},
          {'p':'Jesús te adoro'},
          {'p':'por lo que hiciste en mi'}
        ],
        'bridge-1':[
          {'p':'Digno es el Cordero de Dios,'},
          {'p':'digno es el Rey que la muerte venció'}
        ],
        'bridge-2':[
          {'p':'Digno es el Cordero de Dios,'},
          {'p':'digno, digno, digno'}
        ],
      },
      'chords':{
        'intro':[
          {'4':'A'},
          {'4':'Asus'},
          {'4':'D'},
          {'4':'Dsus'}
        ],
      'verse-1':[
          {'4':'A'},
          {'4':'D'},
          {'4':'F#m'},
          {'2':'E'},
          {'2':'D'}
        ],
        'verse-2':[
          {'4':'A'},
          {'4':'D'},
          {'4':'F#m'},
          {'2':'E'},
          {'2':'D'}
        ],
        'verse-3':[
          {'4':'A'},
          {'4':'D'},
          {'4':'F#m'},
          {'2':'E'},
          {'2':'D'}
        ],
        'verse-4':[
          {'4':'A'},
          {'4':'D'},
          {'4':'F#m'},
          {'2':'E'},
          {'2':'D'}
        ],
        'chorus-1':[
          {'4':'A'},
          {'4':'D'},
          {'4':'F#m'},
          {'4':'E'}
        ],
        'chorus-2':[
          {'4':'A'},
          {'4':'D'},
          {'4':'E'},
          {'4':'D'}
        ],
        'bridge-1':[
          {'4':'A'},
          {'4':'D'},
          {'4':'F#m'},
          {'2':'E'},
          {'2':'D'}
        ],
        'bridge-2':[
          {'4':'A'},
          {'4':'D'},
          {'4':'F#m'},
          {'2':'E'},
          {'2':'D'}
        ]
      }
    }
    ,
    {
      'artist':'Kike Pavón',
      'song':'En Tu Nombre',
      'order':[
        'intro',
        'verse-1',
        'verse-2',
        'pre-chorus',
        'chorus-1',
        'chorus-2',
        'intro',
        'verse-1',
        'verse-2',
        'pre-chorus',
        'chorus-1',
        'chorus-2',
        'chorus-2',
        'chorus-2'
      ],
      'lyrics':{
        'verse-1':[
          {'p':'En tu nombre hoy venimos,'},
          {'p':'estamos tan agradecidos'},
          {'p':'y por eso levantamos la voz y el corazón a ti'}
        ],
        'verse-2':[
          {'p':'La alabanza te entregamos,'},
          {'p':'nuestro corazón postramos,'},
          {'p':'porque tú eres el motivo de nuestra adoración'}
        ],
        'pre-chorus':[
          {'p':'A nadie daremos tu gloria compartida,'},
          {'p':'pues es sólo para ti,'},
          {'p':'pues es sólo para ti'}
        ],
        'chorus-1':[
          {'p':'Levantamos nuestras manos,'},
          {'p':'en tu nombre adoramos,'},
          {'p':'por quién eres, lo que has hecho y lo que harás'},
        ],
        'chorus-2':[
          {'p':'Eres rey y nos postramos,'},
          {'p':'eres Dios y proclamamos,'},
          {'p':'tu grandeza y tu bondad'},
        ]
      },
      'chords':{
        'intro':[
          {'4':'D'},
          {'4':'A'},
          {'4':'C'},
          {'2':'G'},
          {'2':'A'}
        ],
        'verse-1':[
          {'4':'D'},
          {'4':'A'},
          {'4':'C'},
          {'2':'G'},
          {'2':'A'}
        ],
        'verse-2':[
          {'4':'D'},
          {'4':'A'},
          {'4':'C'},
          {'2':'G'},
          {'2':'A'}
        ],
        'pre-chorus':[
          {'4':'Bm'},
          {'4':'G'},
          {'4':'D'},
          {'4':'A'}
        ],
        'chorus-1':[
          {'4':'D'},
          {'4':'A'},
          {'4':'Bm'},
          {'2':'A'},
          {'2':'G'}
        ],
        'chorus-2':[
          {'4':'D'},
          {'4':'A'},
          {'4':'Bm'},
          {'2':'A'},
          {'2':'G'}
        ]
      }
    }
    ,
    {
      'artist':'Evan Craft',
      'song':'Vives en Mí',
      'order':[
        'intro',
        'verse-1-1',
        'verse-1-2',
        'verse-2-1',
        'verse-2-2',
        'pre-chorus',
        'pre-chorus',
        'chorus-1',
        'chorus-2',
        'chorus-1',
        'chorus-2',
        'intro',
        'verse-2-1',
        'verse-2-2',
        'pre-chorus',
        'pre-chorus',
        'chorus-1',
        'chorus-2',
        'chorus-1',
        'chorus-2',
        'bridge-1',
        'bridge-1',
        'bridge-1',
        'bridge-2',
        'pre-chorus',
        'pre-chorus',
        'chorus-1',
        'chorus-2',
        'chorus-1',
        'chorus-2'
      ],
      'lyrics':{
        'verse-1-1':[
          {'p':'Al despertar te cantaré,'},
          {'p':'mis ojos en ti fijaré.'}
        ],
        'verse-1-2':[
          {'p':'Al ritmo de tu corazón,'},
          {'p':'caminaré hacia tu amor.'}
        ],
        'verse-2-1':[
          {'p':'Tu fuego en mi interior'},
          {'p':'ardiendo está,'},
          {'p':'salvaje amor que brillará.'}
        ],
        'verse-2-2':[
          {'p':'Esta pasión, todos verán,'},
          {'p':'tu gloria resplandecerá.'}
        ],
        'pre-chorus':[
          {'p':'Tú nunca me dejarás,'},
          {'p':'tu amor me sostendrá,'},
          {'p':'junto a mí estarás'},
          {'p':'y por siempre brillarás'}
        ],
        'chorus-1':[
          {'p':'Tú vives en mí, vives en mí,'},
          {'p':'soy tuyo para siempre.'}
        ],
        'chorus-2':[
          {'p':'Vives en mí, vives en mí,'},
          {'p':'soy tuyo para siempre.'}
        ],
        'bridge-1':[
          {'p':'Por siempre, por siempre,'},
          {'p':'por siempre en tu amor'}
        ],
        'bridge-2':[
          {'p':'Por siempre, por siempre,'},
          {'p':'por siempre, yo sé que...'}
        ]
      },
      'chords':{
        'intro':[
          {'4':'C'},
          {'4':'D'},
          {'4':'Bm'},
          {'4':'Em'}
        ],
        'verse-1-1':[
          {'4':'C'},
          {'4':'D'},
          {'4':'Bm'},
          {'4':'Em'}
        ],
        'verse-1-2':[
          {'4':'C'},
          {'4':'D'},
          {'4':'Bm'},
          {'4':'Em'}
        ],
        'verse-2-1':[
          {'4':'C'},
          {'4':'D'},
          {'4':'Bm'},
          {'4':'Em'}
        ],
        'verse-2-2':[
          {'4':'C'},
          {'4':'D'},
          {'4':'Bm'},
          {'4':'Em'}
        ],
        'pre-chorus':[
          {'4':'C'},
          {'4':'D'},
          {'4':'Bm'},
          {'4':'Em'}
        ],
        'chorus-1':[
          {'4':'C'},
          {'4':'D'},
          {'4':'Bm'},
          {'4':'Em'}
        ],
        'chorus-2':[
          {'4':'C'},
          {'4':'D'},
          {'4':'Bm'},
          {'4':'Em'}
        ],
        'bridge-1':[
          {'4':'C'},
          {'4':'D'},
          {'4':'Bm'},
          {'4':'Em'}
        ],
        'bridge-2':[
          {'4':'C'},
          {'4':'D'},
          {'4':'Bm'},
          {'4':'Em'}
        ]
      }
    }
    ,
    {
      'artist':'Hillsong Worship',
      'song':'En Esto Creo',
      'order':[
        'intro',
        'intro',
        'verse-1-1',
        'verse-1-2',
        'chorus-1-1',
        'chorus-1-2',
        'chorus-1-3',
        'chorus-1-4',
        'verse-1-1',
        'verse-1-2',
        'chorus-1-1',
        'chorus-1-2',
        'chorus-1-3',
        'chorus-1-4',
        'bridge-1',
        'bridge-2',
        'bridge-1',
        'bridge-2',
        'chorus-1-1',
        'chorus-1-2',
        'chorus-1-3',
        'chorus-1-4',
        'chorus-1-1',
        'chorus-1-2',
        'chorus-1-3',
        'chorus-1-4',
        'chorus-1-4',
        'chorus-1-4',
        'intro',
        'chorus-2-1',
        'chorus-2-2',
        'chorus-2-3',
        'chorus-2-4',
        'chorus-1-1',
        'chorus-1-2',
        'chorus-1-3',
        'chorus-1-4'
      ],
      'lyrics':{
        'verse-1-1':[
          {'p':'Mi Dios y padre eterno,'},
          {'p':'autor de la creacion, poderoso'}
        ],
        'verse-1-2':[
          {'p':'Tu Espíritu intervino,'},
          {'p':'el verbo se encarno, Cristo tu salvas'}
        ],
        'verse-2-1':[
          {'p':'Mi juez y mi abogado,'},
          {'p':'sufriste en la cruz, me has perdonado'}
        ],
        'verse-2-2':[
          {'p':'Bajaste a las tinieblas'},
          {'p':'y en gloria y en poder te levantaste'}
        ],
        'chorus-1-1':[
          {'p':'Creo en nuestro Dios el Padre'},
          {'p':'y en su hijo Cristo el rey'}
        ],
        'chorus-1-2':[
          {'p':'Creo en el Espírito Santo,'},
          {'p':'Dios tres en uno es'}
        ],
        'chorus-1-3':[
          {'p':'Creo que resucitaste,'},
          {'p':'que nos levantaras'}
        ],
        'chorus-1-4':[
          {'p':'Creo en el nombre de Jesus Cristo.'}
        ],
        'bridge-1':[
          {'p':'Creo en ti Jesús.'},
          {'p':'Creo que tu vivo estas.'}
        ],
        'bridge-2':[
          {'p':'Creo que Jesús es el Señor.'},
        ],
        'chorus-2-1':[
          {'p':'Creo en la vida eterna.'},
          {'p':'Creo en la comunión.'}
        ],
        'chorus-2-2':[
          {'p':'Creo que tu pueblo es uno.'},
          {'p':'Creo en tu iglesia Dios.'}
        ],
        'chorus-2-3':[
          {'p':'Creo que resucitaremos'},
          {'p':'y en gloria volverás'}
        ],
        'chorus-2-4':[
          {'p':'Creo en el nombre de Jesus Cristo'}
        ]
      },
      'chords':{
        'intro':[
          {'4':'C'},
          {'4':'Em'},
          {'4':'D'},
          {'4':'G'}
        ],
        'verse-1-1':[
          {'4':'C'},
          {'4':'Em'},
          {'4':'D'},
          {'4':'G'}
        ],
        'verse-1-2':[
          {'4':'C'},
          {'4':'Em'},
          {'4':'D'},
          {'4':'G'}
        ],
        'verse-2-1':[
          {'4':'C'},
          {'4':'Em'},
          {'4':'D'},
          {'4':'G'}
        ],
        'verse-2-2':[
          {'4':'C'},
          {'4':'Em'},
          {'4':'D'},
          {'4':'G'}
        ],
        'chorus-1-1':[
          {'4':'G'},
          {'4':'Am'},
          {'4':'C'},
          {'4':'D'}
        ],
        'chorus-1-2':[
          {'4':'G'},
          {'4':'Am'},
          {'4':'C'},
          {'4':'D'}
        ],
        'chorus-1-3':[
          {'4':'G'},
          {'4':'Am'},
          {'4':'C'},
          {'4':'D'}
        ],
        'chorus-1-4':[
          {'4':'G'},
          {'4':'Am'},
          {'4':'C'},
          {'4':'D'}
        ],
        'chorus-2-1':[
          {'4':'G'},
          {'4':'Am'},
          {'4':'C'},
          {'4':'D'}
        ],
        'chorus-2-2':[
          {'4':'G'},
          {'4':'Am'},
          {'4':'C'},
          {'4':'D'}
        ],
        'chorus-2-3':[
          {'4':'G'},
          {'4':'Am'},
          {'4':'C'},
          {'4':'D'}
        ],
        'chorus-2-4':[
          {'4':'G'},
          {'4':'Am'},
          {'4':'C'},
          {'4':'D'}
        ],
        'bridge-1':[
          {'4':'C'},
          {'4':'Em'},
          {'4':'D'},
          {'4':'G'}
        ],
        'bridge-2':[
          {'4':'C'},
          {'4':'Em'},
          {'4':'D'},
          {'4':'G'}
        ]
      }
    }
    ,
    {
      'artist':'Hillsong United',
      'song':'Reinas por la Eternidad',
      'order':[
        'intro',
        'verse-1-1',
        'verse-1-2',
        'verse-1-3',
        'verse-1-4',
        'verse-2-1',
        'verse-2-2',
        'verse-2-3',
        'verse-2-4',
        'chorus-1-1',
        'chorus-1-2',
        'chorus-1-3',
        'verse-3-1',
        'verse-3-2',
        'verse-3-3',
        'verse-3-4',
        'chorus-1-1',
        'chorus-1-2',
        'chorus-1-3',
        'chorus-1-1',
        'chorus-1-2',
        'chorus-1-3',
        'bridge',
        'bridge',
        'bridge',
        'chorus-1-1',
        'chorus-1-2',
        'chorus-1-3',
        'chorus-1-1',
        'chorus-1-2',
        'chorus-1-3'
      ],
      'lyrics':{
        'verse-1-1':[
          {'p':'Bueno es Dios, bueno es Dios,'},
          {'p':'cuando nada hay bueno en mi'}
        ],
        'verse-1-2':[
          {'p':'Tu amor, tu amor,'},
          {'p':'no se puede resistir'}
        ],
        'verse-1-3':[
          {'p':'Eres luz, eres luz,'},
          {'p':'aun en la oscuridad'}
        ],
        'verse-1-3':[
          {'p':'Eres fiel, eres fiel,'},
          {'p':'has borrado mi maldad '}
        ],
        'verse-2-1':[
          {'p':'Eres paz, eres paz,'},
          {'p':'aun en medio del temor '}
        ],
        'verse-2-2':[
          {'p':'La verdad, la verdad,'},
          {'p':'cuando ando en confusion '}
        ],
        'verse-2-3':[
          {'p':'La razón, la razón,'},
          {'p':'de mi gozo esta en Jesús '}
        ],
        'verse-2-4':[
          {'p':'Él venció, Él venció,'},
          {'p':'a la muerte en la cruz '}
        ],
        'verse-3-1':[
          {'p':'Eres más, eres más,'},
          {'p':'de lo que puedo expresar'}
        ],
        'verse-3-2':[
          {'p':'Eres Rey, eres Rey,'},
          {'p':'la creación proclamará'}
        ],
        'verse-3-3':[
          {'p':'Aquí estás, aquí estás,'},
          {'p':'tu presencia es real'}
        ],
        'verse-3-4':[
          {'p':'Eres Dios, eres Dios,'},
          {'p':'lo pasado dejo atrás'}
        ],
        'chorus-1-1':[
          {'p':'Tú, me abrazas con tu amor,'},
          {'p':'me abrazas con tu amor '}
        ],
        'chorus-1-2':[
          {'p':'Tu gracia me encontró,'},
          {'p':'cambió mi corazón '}
        ],
        'chorus-1-3':[
          {'p':'Nada se puede comparar,'},
          {'p':'reinas por la Eternidad '}
        ],
        'bridge':[
          {'p':'Mi corazón,'},
          {'p':'te cantará,'},
          {'p':'Jesús, Jesús'},
        ]
      },
      'chords':{
      }
    }
    ,
    {
      'artist':'Hillsong United',
      'song':'Tu Amor No Se Rinde',
      'order':[
        'intro',
        'verse-1-1',
        'verse-1-2',
        'verse-1-3',
        'verse-1-4',
        'verse-1-4',
        'pre-chorus-1',
        'pre-chorus-2',
        'chorus',
        'chorus',
        'chorus',
        'chorus',
        'verse-2-1',
        'verse-2-2',
        'verse-2-3',
        'verse-2-4',
        'verse-2-4',
        'bridge-1',
        'bridge-2',
        'chorus',
        'chorus',
        'chorus',
        'chorus'
      ],
      'lyrics':{
        'verse-1-1':[
          {'p':'Un nuevo inicio nos ofreces'}
        ],
        'verse-1-2':[
          {'p':'Los corazones a ti vuelven'}
        ],
        'verse-1-3':[
          {'p':'Tu redención es persistente'}
        ],
        'verse-1-4':[
          {'p':'Constante amor'}
        ],
        'verse-2-1':[
          {'p':'La oscuridad hemos dejado'}
        ],
        'verse-2-2':[
          {'p':'Y por tu luz somos guiados'}
        ],
        'verse-2-3':[
          {'p':'Tu esperanza encontramos'}
        ],
        'verse-2-4':[
          {'p':'Constante amor'}
        ],
        'pre-chorus-1':[
          {'p':'Me llevarás, llevarás,'},
          {'p':'en tus brazos Dios'}
        ],
        'pre-chorus-2':[
          {'p':'Me cubrirás, cubrirás,'},
          {'p':'Con tu eterno amor'}
        ],
        'chorus':[
          {'p':'Tu amor no se rinde.'},
          {'p':'Tu amor no se rinde'}
        ],
        'bridge-1':[
          {'p':'Has abierto nuestros ojos,'},
          {'p':'caminamos en'},
          {'p':'tu libertad'}
        ],
        'bridge-2':[
          {'p':'Sin cesar tú lucharás'},
          {'p':'para salvar la humanidad'}
        ]
      },
      'chords':[
      ]
    }
    ,
    {
      'artist':'Worship Central',
      'song':'Cristo Te Seguiré',
      'order':[
        'intro',
        'verse-1-1',
        'verse-1-2',
        'verse-1-3',
        'chorus-1-1',
        'verse-2-1',
        'verse-2-2',
        'verse-2-3',
        'chorus-1-1',
        'chorus-1-2',
        'chorus-1-2',
        'chorus-1-3',
        'chorus-1-1',
        'chorus-1-2',
        'chorus-1-2',
        'chorus-1-4',
        'chorus-1-2',
        'chorus-1-2',
        'verse-3-1',
        'verse-3-2',
        'verse-3-3',
        'chorus-1-1',
        'verse-4-1',
        'verse-4-2',
        'verse-4-3',
        'chorus-1-1',
        'chorus-1-2',
        'chorus-1-2',
        'chorus-1-3',
        'chorus-1-1',
        'intro',
        'chorus-1-2',
        'chorus-1-2',
        'chorus-1-3',
        'chorus-1-1',
        'chorus-1-2',
        'chorus-1-2',
        'chorus-1-4',
        'chorus-1-1',
        'chorus-1-1',
        'chorus-1-2',
        'chorus-1-2',
        'chorus-1-3',
        'chorus-1-1',
        'chorus-1-2',
        'chorus-1-2',
        'chorus-1-4',
        'chorus-1-1',
        'chorus-1-1'
      ],
      'lyrics':{
        'verse-1-1':[
          {'p':'Cuando estaba perdido en pecado'}
        ],
        'verse-1-2':[
          {'p':'Tu amor me rescató'}
        ],
        'verse-1-3':[
          {'p':'Me salvaste y borraste el pecado'}
        ],
        'verse-2-1':[
          {'p':'Era ciego y hoy veo por gracia'}
        ],
        'verse-2-2':[
          {'p':'Ahora veo en tu luz'}
        ],
        'verse-2-3':[
          {'p':'Me cambiaste y me diste esperanza'}
        ],
        'verse-3-1':[
          {'p':'Creeré y confiaré en tu promesa'}
        ],
        'verse-3-2':[
          {'p':'Tú no me fallarás'}
        ],
        'verse-3-3':[
          {'p':'Por tu amor tengo vida eterna'}
        ],
        'verse-4-1':[
          {'p':'Viviré de victoria en victoria'}
        ],
        'verse-4-2':[
          {'p':'Danzaré en libertad'}
        ],
        'verse-4-3':[
          {'p':'Tu me llevas de gloria en gloria'}
        ],
        'chorus-1-1':[
          {'p':'Cristo, te seguiré'}
        ],
        'chorus-1-2':[
          {'p':'Te seguiré'}
        ],
        'chorus-1-3':[
          {'p':'Tu amor me salvó de la muerte'}
        ],
        'chorus-1-4':[
          {'p':'Con tu luz brillaré para siempre'}
        ]
      },
      'chords':[
      ]
    }
];




var number = {
  '1':'one',
  '2':'two',
  '3':'three',
  '4':'four',
  '5':'five',
  '6':'six'
}

var part = {
  'intro':'Intro',
  'verse-1':'Verso 1',
  'verse-1-1':'Verso 1',
  'verse-2-1':'Verso 2',
  'verse-2':'Verso 2',
  'verse-2-1':'Verso 2',
  'verse-2-2':'Verso 2',
  'verse-3':'Verso 3',
  'verse-4':'Verso 4',
  'chorus-1':'Coro',
  'chorus-1-1':'Coro',
  'chorus-1-2':'Coro',
  'chorus-1-3':'Coro',
  'chorus-1-4':'Coro',
  'chorus-2':'Coro',
  'chorus-2-1':'Coro',
  'chorus-2-2':'Coro',
  'chorus-2-3':'Coro',
  'chorus-2-4':'Coro',
  'bridge':'Puente',
  'bridge-1':'Puente',
  'bridge-2':'Puente'
}

var controls = [
  {'a':'item backward song disabled','i':'icon step backward'},
  {'a':'item play song disabled','i':'icon play'},
  {'a':'item stop song disabled','i':'icon stop'},
  {'a':'item forward song disabled','i':'icon step forward'}
]

var getData = function( artist , song ){
  for( i in data ){
    if( data[i].artist == artist && data[i].song == song ){
      return data[i];
    }
  }
  return null;
}

var getStringData = function( obj ){
  var string = ' ';
  for( i in obj ){
    for( j in obj[i] ){
      string += obj[i][j];
      string += ' ';
    }
  }
  return string;
}
